...
Jeśli tu jesteś, to znaczy, że ktoś cię poprosił o zrobienie Hat'a do MiniModuCard'a.
W tej książce jest cała dokumentacja systemu, ale postaram się streścić najważniejsze
rzeczy, które musisz wiedzieć tutaj.

Zakładam, że dostałeś zadanie w stylu "zrób hat'a, który odbiera GPS", "który
steruje silnikiem BLDC" albo "zrób hata z układem scalonym DRV17182". Jeśli się zgadza, to
możesz czytać dalej.
...
Mamy system, w którym jest płytka z mikroprocesorem (np. STM32F405) i chcemy, żeby coś robiła.
Żeby to było modularne, oddzieliliśmy procesor i układy wyjściowe na osobne płytki. Te osobne
płytki z układami wyjściowymi to są "Hat'y" i ty takiego masz robić.
...
Procesor ma swoje piny (to, co podpinasz Arduino do modułów). One zmieniają swoje napięcia i sterują rzeczami
wyprowadzonymi na złącze ChainBus. Złącze ma konkretne
rzeczy na konkretnych pinach, żeby było
łatwo wymieniać i projektować hat'y.

Te konkretne rzeczy na konkretnych pinach to I2C, SPI i UART. To są takie mega
często stosowane protokoły komunikacyjne. Ich będziesz używać do sterowania układami scalonymi (IC)
na hat'ie.

Twoje zadanie to znaleźć układ, który robi to, co chcesz (np. sterownik termopary) taki, żeby komunikował
się po I2C, SPI albo UART, a nie jakimś dziwnym własnym protokołem. Jak już znajdziesz, to zapytaj się
osoby, od której dostałeś zadanie, czy będzie pasował.

Jak masz ten układ wybrany, to musisz narysować w programie "KiCad", do czego jest podłączony. To jest schematic. Potem
przenosisz go na fizyczną płytkę w tym samym programie. Wtedy masz PCB.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{photos/MMS3_hat_template_schematic.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{photos/MMS3_hat_template_pcb.png}
...
\begin{itemize}
  ...
  \item Nie możesz rzucać Interboard connector, Bus switch, ID eeprom na schematic'u. One muszą zostać takie, jakie są
  \item Nie zmieniaj rozmiaru płytki PCB i nie ruszaj rozstawu śrub montażowych oraz złącz ChainBus.
  \item Możesz usunąć ze schematic'a i PCB złącze XT60, jeśli go nie potrzebujesz (ono jest, jak potrzebujesz dużo prądu)
  \item Na schematic'u możesz się podpiąć do I2C, SPI i UART, jak wychodzą z tej ramki, za bus switchem. Nie ruszaj nic w środku ramki
  \item Schematic jest podzielony na hierarchical sheet'y (np. Bus switch. Te prostokąty, możesz na nie dwa
        razy kliknąć, żeby zobaczyć do środka). Proszę też ich używaj <3
  \item O, i co do hierarchical sheet'ów (schematów hierarchicznych), to możesz zobaczyć, czy na innym hat'ie
        nie ma jakiegoś, który ci się przyda. Są opisane w README
  \item I2C, SPI i UART są w formie BUS connection. Zobacz w ID eeprom, jak wchodzi gruba niebieska linia,
        a w środku rozbija się na dwie małe zielone.
\end{itemize}

Basically musisz dodać swój układ i połączyć z jedną/dwoma/trzema protokołami komunikacyjnymi
wychodzącymi z tej bordowej przerywanej ramki. I nie usuwać/przesuwać ważnych rzeczy.
...
\begin{itemize}
  ...
  \item Do pobierania footprint'ów komponentów z LCSC \url{https://github.com/uPesy/easyeda2kicad.py}
...
Zwykle zamawiamy PCB częściowo zlutowane (basic assembly) z JLCPCB. Trzeba używać komponentów od nich, żeby nie trzeba było
ich później ręcznie wybierać przy zamawianiu. Dodatkowym bonusem jest to, że ładnie widać je na modelu 3D.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{./photos/KiCad_adding_JLCPCB_components.png}
  \caption{Widok z dodawania komponentów KiCad}
  %\label{fig:NoLabel}
\end{figure}

Staraj się wybierać, jak możesz, komponenty "Basic Component". Za każdy inny komponent trzeba zapłacić dodatkowo 1.5\$ za załadowanie
do maszyny pick and place. Zazwyczaj basic będą tylko podstawowe rezystory, kondensatory, diody (w tym LED), oscylatory i parę innych. Pełną listę możesz
znaleźć, wchodząc na stronę \url{https://jlcpcb.com/parts} i odfiltrować tylko te basic. Najprawdopodobniej lutowane będą tylko te basic
więc im więcej takich użyjesz tym mniej zabawy potem z kupowaniem i lutowaniem części.
...
do tego plugina easyeda2kicad.

A więc, procedura, której ja używam, wygląda tak:
\begin{itemize}
  \item Znajdź swój komponent na \url{https://www.lcsc.com}
  \item Otwórz CMD (konsolę) w \textbf{folder\_twojego\_hat'a}/hardware/lib (możesz wejść do lib, kliknąć tam na górze, jak jest napisane,
        w jakim folderze jesteś (path) w pustym miejscu, tak żeby ci się podświetliło na niebiesko, wpisać "CMD" i dać Enter)
  \item Użyj komendy: \textbf{easyeda2kicad --lcsc\_id <ID\_komponentu\_LCSC> --full --output ./<nazwa\_folderu>}

\end{itemize}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{./photos/using_easyeda2kicad.png}
  \caption{Komenda do pobrania komponentu przez easyeda2kicad}
  %\label{fig:NoLabel}
\end{figure}

\begin{itemize}

  \item Powinno ci się pobrać plik \textbf{nazwa\_folderu.kicad\_sym} i foldery \textbf{nazwa\_folderu.3dshapes} oraz \textbf{nazwa\_folderu.pretty}. Jeśli ich nie masz, to w
        złym miejscu otwarłeś CMD.
  \item Przenieś .kicad\_sym oraz folder .3dshapes do folderu .pretty
  \item Otwórz swój projekt (dwa razy kliknij na .kicad\_pro)
  \item Załaduj symbol -> wejdź w Symbol Editor, w lewym górnym wybierz \text{File} -> \text{Add library} i dodaj plik .kicad\_sym jako "Add new library to project library table"
  \item Teraz załaduj footprint -> pójdź z powrotem do projektu, wejdź w Footprint Editor, i tak samo jak wcześniej, tylko wybierz folder .pretty
  \item Gotowe! Teraz możesz wejść do swojego projektu (widok schematic) i dodać świeżo zaimportowany komponent. Powinien mieć od razu footprint i model 3D.
\end{itemize}
...
\begin{itemize}
  ...
  \item Jeśli układ, który wybrałeś, ma jakieś piny interrupt, to je ignorujesz,
        zostaw je floating (do niczego niepodpięte). Jeśli ma jakieś piny konfiguracyjne, to daj je na zworki goldpin na
        boku płytki (jeśli trzeba je często przełączać - goldpin kątowy jest spoko) albo przez zworkę lutowalną/rezystor na samej płytce (jeśli mniej często
        trzeba je przełączać). Tutaj musisz trochę pomyśleć, jak to musisz zrobić, żeby było dobrze. Zapytaj się kogoś, jak nie wiesz
  \item Jako że nie ma wyprowadzonych zwykłych pinów GPIO do hat'a, tylko same protokoły komunikacyjne, to może ci ich brakować.
        Jeśli będziesz ich potrzebował, to daj expander GPIO po I2C
  \item Robienie schematic'a (co do czego jest podłączone) nie jest trudne. Basically musisz wziąć jakiś
        "Typical design" z noty katalogowej układu (ang. datasheet) albo płytkę, którą już ktoś zrobił
  \item Jak robisz fork'a, to chcesz, żeby właścicielem repo była organizacja Konar'u (np. Hefajstos), a nie ty,
        bo jak jest na ciebie, to nie mam dostępu, żeby poprawić płytkę, jak ją sprawdzam
  ...
  \item Jeśli szukasz układu scalonego i są tylko z własnym
        protokołem albo reszta jest droga, to możesz dać po drodze innego proca, np. CH32V003, który to przetłumaczy na I2C/SPI.
        Napisz do mnie albo innego ogarniętego elektronika, żeby ci to wytłumaczył
  \item Komponenty zamawiamy zwykle albo z TME, Mouser, albo LCSC. TME jest preferowane, ale jest drogo i mało komponentów
\end{itemize}
...
\begin{itemize}
  \item \url{https://www.youtube.com/@PhilsLab}
...
\subsection{Overview}
Bazą dużego ModuCard'a jest backplane - duża płytka pcb z miejscami na karty. Do niego montuje się karty które już faktycznie coś robią.

Karty komunikują się ze sobą po 2x CAN, 2x strobe + mają jeszcze podłączenie USB do karty komputera pokładowego. Do tego garść GPIO i troche pinów konfiguracji.

Dalej, są same karty. Na nich jest cała faktyczna elektronika którą
potrzebuesz - mikroprocesory, czujniki, komputery, zasilacze etc.

Są 3x typy kart

1. Karta komputer pokładowy -> Np. Rasperry Pi 5 czy jakiś minikomputer Jetson. Do niego raportują karty, on przetważa i wysterowywuje odpowiednio kartami 
...
Dla kompletnego systemu potrzebujesz

1x Karte komputera pokładowego - wsadszasz ją w pierwszy slot backplane'a

1 lub więcej Karte zasilacza - wsadasz w dowolny slot

I ile ci się zmieści kart użytkowych - dowolny slot

\subsubsection{ModuCard - Karta}
Karta to nazwa na płytke z faktyczną elektroniką. Jedna karta to jednen cały kompletny system - Mikrokontroler i peryferia. Zwykłe karty Raportują po CAN i USB do Karty komputera pokładowego który steruje systemem. Więcej w sekcjach niżej

\subsubsection{ModuCard - Backplane}
Backplane to duża płytka z kilkoma (np. 6) slotami na karte. Można je łączyć szeregowo ze sobą (czyli połączyć 2x backplane'y 6slot i uzyskać jeden duży backplane 12slot). 

Basically możesz o nim myśleć jako dużym USB hub'ie gdzie wpinasz karty jak pendrive'y.
...
Pierwsza (as in: trzeba ją włożyć do pierwszego slot'u backplane'a. Reszta kart obojętnie) i najważniejsza karta w systemie. To ona steruje wszystkim.

Jest na niej jakiś minikomputer, Np. raspberry Pi, Nvidia Jetson czy to tam potrzebujesz. On odczytuje raporty od kart po Can'ie, rozmawia z nimi po USB i przetważa te dane.

Możesz go zaprogramować jako
1. Komputer pokładowy - no, komputer pokładowy. Przetważa dane i tak jak pisałem
2. Gateway - Odczytuje dane z kart i wysyła po ethernecie do innnego, dużego komputera np. stacjonarnego z mocnym procem i GPU. Wtedy komputer pokładowy robi jako fancy przetwornik ETH <-> CAN.
...
Obsługuje sekwencjonowanie zasilania czyli odpalanie po kolei poszczegółnych szyn i raportowanie ich stanu do komputera pokładowego
\subsection{Karta użytkowa}
I karta najbliższa twojemu sercu (chyba że jesteś programistą ROS'a) czyli karta użytkowa. Na niej jest ten cały ambaras. No to lecimy

Na niej jest przetworka 48V -> 5V czy inne szyny które są potrzebne dla embedded.

Masz na niej mikrokontroler który gada po CAN i USB oraz używa reszty GPIO do interakcji z\dots

Samymi peryferiami -> konwertery ADC, sterowniki, czujniki i wszystko

\section{Duży ModuCard - Rola Backplane'a}

Są na nim wbudowane USB hub'y żeby komputer pokładowy mógł komunikować się z każdą kartą.

Przenosi zasilanie też. Can też. USB i strobe też.

Powiedz o GPIO konfiguracyjnych i GPIO L R

Nadal trzeba pamiętać żeby do pierwszego slotu dać komputer pokładowy

\section{Do czego używać GPIO L/R}
Do niczego lol. Rób dobre karty (jak widać dokumentacja dużego Dużego ModuCard'a jest WIP. Jak znowu będe jechał pociągiem to ją skończe ładnie)
\section{Co to strobe}
Strobe? Jaki strobe? John Strobe?
...
Mały task - mały moducard. Duży task - duży moducard
\section{Pomiędzy systemami - Karta adapter i ChainLink} \label{karta_adapter}
Important section. Much writting.
...
\section{Wymiary}
Jeśli jesteś mechanikiem zespołu, to najlepszą metodą na złapanie wymiarów jest poproszenie elektronika, żeby wyeksportował ci PCB do .step razem z komponentami.

Ta sekcja jest bardziej pod kątem utrzymania standardu.

Wymiary do śrub montażowych albo keepout zone'ów (to jest miejsce, gdzie elektronik nie może stawiać komponentów i ty możesz tam sobie coś zamodelować do złapania hata) - są symetryczne, więc paru wymiarów nie podałem.
