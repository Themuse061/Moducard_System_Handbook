...
W tej książce jest cała dokumentacja systemu, ale postaram się streścić najwazniejsze
rzeczy które musisz wiedzieć tutaj.

Zakładam że dostałeś zadanie w stylu "zrób hat'a który odbiera GPS", "który
steruje silnikem BLDC" albo "zrób hata z układem scalonym DRV17182". Jeśli się zgadza, to
możesz czytać dalej

\subsection{Co musisz wiedzieć}
Powinieneś przeczytać Primer (\ref{Primer}), zasade działana MiniModuCard'a (\ref{MiniModuCard - zasada działania}) oraz sekcje o Hat'ach (\ref{MiniModuCard - Hat}).

Pozwole sobie założyć że tego nie zrobiłeś, i na szybko jeszcze raz to opisze
...
Procesor ma swoje pin'y (to co podpinasz arduino do modułów). One zmieniają swoje napiecia i sterują rzeczami
wyprowadzone na zlącze ChainBus. Złącze ma konkretne
rzeczy na konkretnyh pinach żeby było
...
Twoje zadanie to znaleźć układ który robi to co chesz (np. sterownik termopary) taki, żeby komunikował
się po I2C SPI albo UART, a nie jakimś dziwny własnym protokołem. Jak znajdziesz już to zapytaj się
...
przenosisz go na fizyczna płytke w tym samym programie. Wtedy masz PCB.
...
Żeby było łatwiej projektować to zrobiłem projekt z podstawowaymi elementami hat'a na githubie
...
\begin{itemize}
  \item Masz już zrobione złącze chainbus, adresowanie i reszte
  ...
  \item Na schematic'u możesz się popdpiać do I2C, SPI i UART jak wychodzą z tej ramki, za bus switch'em. Nie ruszaj nic w środku ramki
  \item schematic jest podzielony na  hierarhical sheet'y (np. Bus switch. Te prostokąty, możesz na nie dwa
        razy kliknąć żeby zobaczyć do środka). Prosze też ich używaj <3
  \item O, i co do hierarhical sheet'ów (schematów hierarhicznych) to możesz zobaczyć czy na innym hat'ie
        nie ma jakiegoś który ci się przyda. Zą opisane w readme
  \item I2C, SPI i UART są w formie BUS connection. Zobacz w ID eeprom jak wchodzi gruba niebisa linia,
        a w środku rozbija się an dwie małe zielone.
\end{itemize}

Basically musisz dodać swój układ i połączyć z jedną/dwoma/trzema protokołami komunikacyjnymi
wychodzącymi z tej bordowej prezrywanej ramki. I nie usuwać/przesuwać ważnych rzeczy

Pare jeszcze rzeczy jest napisane w readme template'a
...
Pełne tłumaczenie od zera jest out of scope tego dokumentu (wierze że znajdziesz jakiś fajny poradnik na yt), ale

\subsection{Pobieranie symboli i footprint'ów - pasywki}
(symbol - element na schematic'u. Jakie ma połączenia. Footpint - element na PCB. jaki ma fizyczny kształt)

Zwykle zamawiamy PCB częściowo zlutowane (basic assembly) z JLCPCB. Trzeba używać komponentów od nich żeby nie trzeba było
ich później ręcznie wybierać przy zamawianu. Dodatkowym bonusem jest to że ładnie widać je na modelu 3D.
...
do maszny pick and place. Zazwyczaj basic będą tylko podstawowe rezystory, kondensatory, diody (w tym LED), oscylatory i pare innych. Pełną liste możesz
znaleźć wchodząc na strone \url{https://jlcpcb.com/parts} i odfiltrować tylko te basic.
...
Jeśli nie ma twojego komponentu od automatycznie pobranego do KiCad'a (czyli większość nie-basic) musisz go ręcznie pobrać. Używamy
...
\begin{itemize}
  ...
  \item otwórz CMD (konsole) w \textbf{folder\_twojego\_hat'a}/hardware/lib (możesz wejśc do lib, kliknąć tam na górze jak jest napisane
...
\begin{itemize}
  ...
  \item Teraz załaduj footprint -> pójdź spowrotem do projektu, wejdź w footprint editor, i tak samo jak wcześnij tylko wybierz folder .pretty
  \item Gotowe! Teraz możesz wejść do swojego projektu (widok schematic) i dodać świeżo zaimportowany komponent. Powienen mieć od razu footprint i model 3D.
\end{itemize}



\subsection{Pare moich komentarzy}
\begin{itemize}
  ...
  \item Pamiętaj podpisać jaki to hat, która wersja i kto ją zrobił na płytce na silkscreen
  \item W kicadzie jest widok 3D jak otowórz PCB. Zobacz go sobie
  ...
  \item Jeśli układ który wybrałeś ma jakieś piny interrupt to je ignorujesz,
        zostaw je floating (do niczego nie podpięte). Jeśli ma jakieś piny konfuguracyjne to daj je na zworki goldpin na
        boku płytki (jeśli trzeba je często przełączać. goldpin kątowy jest spoko) albo przez zworke lutowalną/rezystor na samej płytce (jeśli mniej często
        trzeba je przełączać). Tutaj musisz troche pomyśleć jak to musisz zrobić żeby było dobrze. Zapytaj się kogoś jak nie wiesz
  ...
  \item Robienie schematic'a (co do czego jest podłaczone) nie jest trudne. Basically musisz jakąś
        "Typical design" z noty katalogowej układu (ang. datasheet) albo płytke którą już ktoś zrobił
  \item jak robisz fork'a, to chcesz żeby właścicelem repo była organizacja Konar'u (np. Hefajstos) a nie ty
        bo jak jest na ciebie to nie mam dostępu żeby poprawić płytke jak ją sprawdzam
  \item  złącza wyjściowe (to po co robisz płytke) daj jako kątowe JST-XH 2.5mm/2.54mm (któreś z tych dwuch)
  \item  Złącza konfiguracyjne (jeśli jakieś masz) daj na prawej krawędzi hat'a. Reszte rzeczy na dolną. Lewą zostaw pustą
...
\subsubsection{spoko miejsca do znalezienia układu i dokumentacji}
\begin{itemize}
...
To tyle z tłumaczenia. Jeśli jest coś czego nie rozumiesz, myślisz że można lepiej wytłumaczyć albo brakuje zdjęć to prosze \textbf{prosze}
napisz mi co. Imo bardzo ważne jest żeby ta część dokumentacji była zrozumiała, bo poczatkujący będą ją próbowali zrozumieć.
