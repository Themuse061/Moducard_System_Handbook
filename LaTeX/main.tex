...
wyprowadzone na zlącze. Złącze ma konkretne
rzeczy na konkretnyh pinach w standardzie który wymyśliliśmy (ChainBus) żebyśmy mogli
łatwo wymieniać i projektować hat'y.

Te konkretne rzeczy na konkretnych pinach to I2C, SPI i UART. To są takie mega
często stosowane protokoły komunikacyjne. Ich będziesz używać do sterowania układami scalonymi
na hat'ie.

Czyli musisz znaleźć układ który robi to co chesz (np. sterownik termopary) żeby komunikował
się po I2C SPI albo UART, a nie jakimś dziwny własnym protokołem.(Jak nie ma opcji i są tylko z własnym
protokołem albo reszta jest droga, to muszisz dać po drodze innego proca np. ch32v003 który to ogarnie.
Napisz do mnie albo innego elektronika żeby  ci to wytłumaczył)

Jak masz ten układ wybrany, to musisz narysować w programie KiCad do czego jest podłączony. To jest schematic. Potem
przenosisz na fizyczna płytke w tym samym programie. to jest PCB.
...
%\hyperlink{Link do repo}{github.com/KoNaR-Hefajstos/MMS3_hat_templates}
...
\begin{itemize}
  ...
  \item Nie możesz rzucać Interboard connector, Bus switch, ID eeprom na schematic'u i śrub montażowych w lewym dolnym
  \item Możesz zmienić przetwornice 5V -> 3V3 jeśli potrzebujesz i wiesz co robisz
  \item Możesz usunąć ze schematic'a i PCB złącze XT60 jeśli go nie potrzebujesz
  \item Na schematic'u możesz się popdpiać do I2C, SPI i UART jak wychodzą z tej ramki, za bus switch'em
  \item schematic jest podzielony na  hierarhical sheet'y (np. Bus switch. Te prostokąty, możesz na nie dwa
        razy kliknąć żeby zobaczyć do środka)
  \item I2C, SPI i UART są w formie BUS connection. Zobacz w ID eeprom jak wchodzi gruba niebisa linia,
        a w środku rozbija się an dwie małe zielone. Tak działają bus'y
  \item Nie przesuwaj zablokowanych rzeczy na PCB
\end{itemize}
...
\begin{itemize}
  \item Musisz zainstalować KiCad'a
...
Pełne tłumaczenie od zera jest out of scope tego dokumentu (wierze że znajdziesz poradnik na yt), ale

\subsubsection{Pare moich komentarzy}
\begin{itemize}
  ...
  \item Pamiętaj podpisać jaki to hat, która wersja i kto to robił na płtce na silkscreen
  ...
  \item Jeśli układ który wybrałeś ma jakieś piny interrupt to je ignorujesz,
        zostaw floating (do niczego nie podpięte). Jeśli ma jakieś piny konfuguracyjne to daj je na zworki goldpin na
        boku płytki (jeśli trzeba je często przełączać) albo przez zworke lutowalną/rezystor na samej płytce (jeśli mniej często
        trzeba je przełączać). Tutaj musisz troche pomyśleć jak to musisz zrobić żeby było dobrze
  \item Jako że nie ma wyprowadzonych zwykłych pinów do hat'a, tylko same protokoły komunikacyjne
  ...
  \item jak robisz fork'a, to chcesz żeby właścicelem repo była organizacja Konar'u (np. Hefajstos) a nie ty
        bo jak jest ciebie to nie mam dostępu żeby poprawić płytke jak na nią patrze
\end{itemize}
...
\subsubsection{Spoko filmy/źródła o projektoaniu schematic'ów/PCB}
\begin{itemize}
  \item
\end{itemize}
